...
var SettingsManager = 
{
	...
	FileName: "monitor.ini",	// PW: I like my name better.
	LocalFolder: "",
	...
	getFullPath: function () 
    {
        ...
        {
            debugOut("getFullPath: "+error.name+" - "+error.message);
        }
        return fullPath;
	},
	...
	getValue: function (group, key, defaultValue) {
		...
		if ( useGlobalSettingsFile == false ) {
			val = System.Gadget.Settings.readString(key);
			if ( val == "")
				return defaultValue;
			else
				return val;
	...
	getIniString: function() {
	    ...
	    /// </remarks>
		var now = new Date();
		var ini = "; Automatically generated by NasMonitor\n; " + now.toLocaleString() + "\n\n";
		var numGroups = this._groups.length;
